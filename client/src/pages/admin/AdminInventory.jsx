    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-neutral-800 pb-4">
        ...
        <div className="flex gap-2">
          <button onClick={exportExcel} className="flex items-center gap-1.5 px-4 py-2 bg-red-600/20 border border-red-600/40 text-emerald-300 font-bold rounded-none clip-path-rog text-xs hover:bg-red-600/30 transition">
            <Download className="w-3.5 h-3.5" /> Xuất Excel
          </button>
          <button onClick={loadInventory} className="flex items-center gap-1.5 px-4 py-2 bg-black border border-neutral-800 text-neutral-300 font-bold rounded-none clip-path-rog text-xs hover:border-red-600/50 transition">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-neutral-900 border border-neutral-800 clip-path-rog p-4 rounded-none clip-path-rog border-l-4 border-red-600">
        ...
        <div className="bg-neutral-900 border border-neutral-800 clip-path-rog p-4 rounded-none clip-path-rog border-l-4 border-amber-500">
        ...
        <div className="bg-neutral-900 border border-neutral-800 clip-path-rog p-4 rounded-none clip-path-rog border-l-4 border-rose-500">
      ...
      <div className="flex items-center gap-2 border-b border-neutral-800 pb-3">
        <button
          ...
          className={`px-4 py-2 rounded-none clip-path-rog text-xs font-bold ${activeTab === 'all' ? 'bg-red-600 text-white font-bold tracking-widest uppercase' : 'bg-black text-neutral-300 hover:bg-neutral-900'}`}
        >
          Tất Cả Sản Phẩm ({totalItems})
        </button>
        <button
          onClick={() => setActiveTab('low')}
          className={`px-4 py-2 rounded-none clip-path-rog text-xs font-bold flex items-center gap-1.5 ${activeTab === 'low' ? 'bg-amber-500 text-white font-bold tracking-widest uppercase' : 'bg-black text-neutral-300 hover:bg-neutral-900'}`}
        >
      ...
      <div className="bg-neutral-900 border border-neutral-800 clip-path-rog rounded-none clip-path-rog p-4 overflow-x-auto">
        <table className="w-full text-left text-sm text-neutral-300">
          ...
          <tbody className="divide-y divide-slate-800/60">
            {loading ? (
              <tr><td colSpan="5" className="py-12 text-center"><Loader2 className="w-5 h-5 animate-spin mx-auto text-red-500" /></td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan="5" className="py-12 text-center text-slate-500">Không có dữ liệu</td></tr>
            ) : filtered.map(prod => {
              const stock = Number(prod.stock) || 0;
              return (
                <tr key={prod.id} className="hover:bg-black/40">
                  <td className="py-3 px-4 flex items-center gap-3 min-w-0">
                    <img src={resolveImage(prod.image_url)} onError={onImageError} className="w-10 h-10 rounded-none clip-path-rog object-cover bg-neutral-900" alt="" />
                    <span className="font-semibold text-white truncate">{prod.name}</span>
                  </td>
                  <td className="py-3 px-4 text-xs text-neutral-400">{prod.sku || 'Chưa có SKU'}</td>
                  ...
                  <td className="py-3 px-4">
                    {stock === 0 ? (
                      ...
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-red-600/20 text-emerald-300">CÒN HÀNG</span>
                    )}
                  </td>
                  <td className="py-3 px-4 text-right space-x-1">
                    <button onClick={() => openAdjust(prod, 'in')} title="Nhập kho" className="p-2 bg-red-600/10 text-emerald-300 hover:bg-red-600/20 rounded-lg"><Plus className="w-3.5 h-3.5" /></button>
                    ...
                    <button onClick={() => openAdjust(prod, 'adjustment')} title="Điều chỉnh" className="p-2 bg-neutral-900 text-red-400 hover:bg-slate-700 rounded-lg"><Settings2 className="w-3.5 h-3.5" /></button>
                    <button onClick={() => openHistory(prod)} title="Lịch sử" className="p-2 bg-neutral-900 text-neutral-300 hover:bg-slate-700 rounded-lg"><History className="w-3.5 h-3.5" /></button>
                  </td>
                </tr>
              );
            })}
      ...
      {showStockModal && adjustProduct && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="bg-neutral-900 border border-neutral-800 clip-path-rog p-6 rounded-none clip-path-rog w-full max-w-md space-y-4 border border-red-600/30">
            <div className="border-b border-neutral-800 pb-3">
              <h3 className="text-lg font-bold text-white">
                {stockForm.type === 'in' ? 'Nhập Kho' : stockForm.type === 'out' ? 'Xuất Kho' : 'Điều Chỉnh'} - {adjustProduct.name}
              </h3>
              <p className="text-xs text-neutral-400 mt-1">Hiện tại: <span className="text-red-400 font-bold">{adjustProduct.stock}</span></p>
            </div>

            <form onSubmit={submitStockAdjustment} className="space-y-3">
              <div>
                <label className="text-xs text-neutral-400">Số lượng *</label>
                <input
                  type="number" required min="1" value={stockForm.quantity}
                  onChange={(e) => setStockForm({...stockForm, quantity: e.target.value})}
                  className="w-full px-3 py-2 bg-black border border-neutral-800 rounded-none clip-path-rog text-sm"
              ...
              {stockForm.quantity && adjustProduct && (
                <div className="p-3 rounded-none clip-path-rog bg-black/60 text-xs">
                  <span className="text-neutral-400">Tồn kho sau: </span>
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-neutral-800">
                <button type="button" onClick={() => setShowStockModal(false)} className="px-4 py-2 bg-neutral-900 text-neutral-300 font-semibold rounded-none clip-path-rog text-xs">Hủy</button>
                <button type="submit" className="px-5 py-2 bg-red-600 text-white font-bold tracking-widest uppercase font-bold rounded-none clip-path-rog text-xs">Xác Nhận</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showHistoryModal && adjustProduct && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="bg-neutral-900 border border-neutral-800 clip-path-rog p-6 rounded-none clip-path-rog w-full max-w-2xl max-h-[80vh] overflow-y-auto space-y-3 border border-red-600/30">
            <div className="flex items-center justify-between border-b border-neutral-800 pb-3">
              <h3 className="text-lg font-bold text-white">Lịch Sử - {adjustProduct.name}</h3>
              <button onClick={() => setShowHistoryModal(false)} className="text-neutral-400 hover:text-white">✕</button>
            </div>
            {historyLoading ? (
              ...
              <>
                <div className="space-y-2">
                  {history.map((h, i) => (
                    <div key={h.id || i} className="p-3 bg-black/60 rounded-none clip-path-rog flex items-center gap-3">
                      <div className={`p-2 rounded-lg ${
                        h.type === 'in' ? 'bg-red-600/10 text-emerald-300' :
                ...
                {historyTotalPages > 1 && (
                  <div className="flex justify-center gap-2 pt-3 border-t border-neutral-800">
                    {Array.from({length: historyTotalPages}, (_, i) => i + 1).map(p => (
                      <button key={p} onClick={() => loadHistory(adjustProduct.id, p)}
                        className={`w-9 h-9 rounded-lg text-xs font-bold ${historyPage === p ? 'bg-red-600 text-white font-bold tracking-widest uppercase' : 'bg-black text-neutral-400 border border-neutral-800'}`}>{p}</button>
                    ))}
                  </div>
                )}
              </>
