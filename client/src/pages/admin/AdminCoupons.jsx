...
export default function AdminCoupons() {
  ...
  const getCouponStatus = (coupon) => {
    if (coupon.is_active === false) return { label: 'VÔ HIỆU', class: 'bg-slate-700/50 text-slate-400 border-slate-600' };
    ...
    return { label: 'ĐANG HOẠT ĐỘNG', class: 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30' };
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-white flex items-center gap-2">
            <Tag className="w-7 h-7 text-cyan-400" /> Quản Lý Mã Giảm Giá
          </h1>
          <p className="text-slate-400 text-sm mt-1">
            Tổng: <span className="text-cyan-400 font-bold">{pagination.totalItems}</span> mã
            {pagination.totalPages > 1 && <span> • Trang {pagination.currentPage}/{pagination.totalPages}</span>}
          </p>
        </div>
        <div className="flex gap-2">
          <button onClick={loadCoupons} className="p-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl transition-colors">
            <RefreshCw className="w-4 h-4" />
          </button>
          <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-cyan-400 to-sky-400 text-slate-950 font-bold rounded-xl text-sm hover:shadow-lg hover:shadow-cyan-500/25 transition-all">
      ...
      <div className="glass-card rounded-2xl p-4 flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-slate-900 border border-slate-800 rounded-xl text-sm text-slate-100 focus:outline-none focus:border-cyan-500 transition-colors"
          />
        </div>
        <select
          value={statusFilter}
          onChange={(e) => { setStatusFilter(e.target.value); setPage(1); }}
          className="px-4 py-2.5 bg-slate-900 border border-slate-800 rounded-xl text-sm text-slate-200 focus:outline-none focus:border-cyan-500 cursor-pointer"
        >
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan="7" className="py-16 text-center">
                    ...
                    <button onClick={openAdd} className="mt-3 text-cyan-400 text-sm hover:underline">Tạo mã đầu tiên</button>
                  </td>
                </tr>
              ) : coupons.map(c => {
                ...
                return (
                  <tr key={c.id} className="hover:bg-slate-900/40 transition-colors">
                    <td className="py-4 px-4">
                      <div className="font-bold text-cyan-300 tracking-wider">{c.code}</div>
                    ...
                    <td className="py-4 px-4">
                      ...
                      <div className="w-full h-1.5 bg-slate-800 rounded-full mt-2">
                        <div
                          className={`h-full rounded-full transition-all ${usagePercent >= 90 ? 'bg-rose-500' : usagePercent >= 70 ? 'bg-amber-500' : 'bg-cyan-500'}`}
                    ...
                    <td className="py-4 px-4">
                      <div className="text-sm text-slate-400">
                        {c.valid_to ? (
                          ...
                          <span className="text-emerald-400">Không giới hạn</span>
                        )}
                      </div>
                    </td>
                    <td className="py-4 px-4">
                    ...
                    <td className="py-4 px-4 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => openEdit(c)} className="p-2 bg-slate-800 hover:bg-cyan-500/20 text-cyan-400 rounded-lg transition-colors" title="Sửa">
                          <Edit className="w-4 h-4" />
                        </button>
                        <button onClick={() => handleDelete(c.id, c.code)} className="p-2 bg-slate-800 hover:bg-rose-500/20 text-rose-400 rounded-lg transition-colors" title="Xóa">
      ...
      {pagination.totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          <button
            onClick={() => setPage(1)}
            disabled={page === 1}
            className="px-3 py-2 bg-slate-900 border border-slate-800 rounded-lg text-xs text-slate-400 hover:border-cyan-500/40 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
            « Đầu
          </button>
          <button
          ...
          <div className="px-4 py-2 bg-slate-900 border border-slate-800 rounded-lg">
            <span className="text-sm text-cyan-300 font-bold">{pagination.currentPage}</span>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="glass-card p-6 rounded-3xl w-full max-w-lg space-y-5 border border-cyan-500/30 max-h-[90vh] overflow-y-auto">
            {/* Modal Header */}
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              ...
              <button onClick={() => setModalOpen(false)} className="p-1 hover:bg-slate-800 rounded-lg transition-colors">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-4 py-2.5 bg-slate-900 border rounded-xl text-sm uppercase tracking-wider ${errors.code ? 'border-rose-500' : 'border-slate-800 focus:border-cyan-500'} transition-colors`}
                />
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-2.5 bg-slate-900 border rounded-xl text-sm ${errors.discount_percent ? 'border-rose-500' : 'border-slate-800 focus:border-cyan-500'} transition-colors`}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-2.5 bg-slate-900 border rounded-xl text-sm ${errors.max_discount ? 'border-rose-500' : 'border-slate-800 focus:border-cyan-500'} transition-colors`}
                  />
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-slate-900 border border-slate-800 rounded-xl text-sm focus:border-cyan-500 transition-colors"
                />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-2.5 bg-slate-900 border rounded-xl text-sm ${errors.min_order_amount ? 'border-rose-500' : 'border-slate-800 focus:border-cyan-500'} transition-colors`}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-2.5 bg-slate-900 border rounded-xl text-sm ${errors.usage_limit ? 'border-rose-500' : 'border-slate-800 focus:border-cyan-500'} transition-colors`}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-2.5 bg-slate-900 border rounded-xl text-sm ${errors.usage_per_user ? 'border-rose-500' : 'border-slate-800 focus:border-cyan-500'} transition-colors`}
                  />
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-2.5 bg-slate-900 border rounded-xl text-sm ${errors.expires_at ? 'border-rose-500' : 'border-slate-800 focus:border-cyan-500'} transition-colors`}
                  />
              ...
              <div className="bg-slate-900/50 rounded-xl p-3 space-y-3">
                <label className="flex items-center gap-3 cursor-pointer">
                  <div className="relative">
                    ...
                    />
                    <div className="w-10 h-6 bg-slate-700 rounded-full peer peer-checked:bg-cyan-500 transition-colors"></div>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-800">
                <button
                  ...
                  className="px-5 py-2.5 bg-slate-800 text-slate-300 font-semibold rounded-xl text-sm hover:bg-slate-700 transition-colors"
                >
                  Hủy
                </button>
                <button
                  type="submit"
                  disabled={saving}
                  className="px-6 py-2.5 bg-gradient-to-r from-cyan-400 to-sky-400 text-slate-950 font-bold rounded-xl text-sm disabled:opacity-50 flex items-center gap-2 hover:shadow-lg hover:shadow-cyan-500/25 transition-all"
                >
