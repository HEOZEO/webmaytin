...
function SummaryStat({ icon: Icon, label, value, color = 'cyan' }) {
  const colorMap = {
    cyan:    'from-cyan-500/15 to-cyan-500/0 border-cyan-500/30 text-cyan-300',
    amber:   'from-amber-500/15 to-amber-500/0 border-amber-500/30 text-amber-300',
    purple:  'from-purple-500/15 to-purple-500/0 border-purple-500/30 text-purple-300',
  ...
  return (
    <div className={`bg-gradient-to-br ${cls} border rounded-2xl p-3.5 flex items-center gap-3`}>
...
function Pagination({ currentPage, totalPages, onPageChange }) {
  ...
  const btnIdle = "bg-slate-800 hover:bg-slate-700 text-slate-200";
  const btnActive = "bg-gradient-to-r from-cyan-400 to-sky-400 text-slate-950 shadow-md shadow-cyan-500/20";
  const btnDisabled = "bg-slate-900 text-slate-600 cursor-not-allowed";
...
function CustomerOrderCard({ order, onViewDetail, detailLoading, onCancelled }) {
  ...
  return (
    <div className="glass-card rounded-2xl overflow-hidden transition-all hover:border-cyan-500/30">
      {/* Header row */}
      <div className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800">
        <div className="flex items-center gap-3 flex-wrap min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-base font-bold text-cyan-400">#{order.id}</span>
          </div>
        ...
        <div className="flex items-center gap-4">
          <div className="text-right">
            ...
            <p className="text-base sm:text-lg font-black text-cyan-400">{formatVND(order.final_amount)}</p>
          </div>
          <button
            onClick={onViewDetail}
            disabled={detailLoading}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-cyan-500/10 border border-cyan-500/30 text-cyan-300 rounded-lg text-xs font-semibold hover:bg-cyan-500/20 transition-colors"
          >
            {detailLoading
      ...
      {/* Date + payment method */}
      <div className="px-4 sm:px-5 py-2 flex items-center justify-between text-xs text-slate-400 bg-slate-950/40">
        <div className="flex items-center gap-2">
      ...
      {!isCancelled && currentStepIdx >= 0 && (
        <div className="px-4 sm:px-5 py-3 border-b border-slate-800 bg-slate-900/30">
                ...
                <React.Fragment key={step}>
                  <div className="flex flex-col items-center gap-1 min-w-0">
                    <div
                      className={`flex items-center justify-center w-8 h-8 rounded-full transition-all ${
                        isCurrent
                          ? 'bg-cyan-500 text-slate-950 ring-4 ring-cyan-500/20 scale-110'
                          : isPast
                    ...
                    <span className={`text-[9px] font-semibold text-center leading-tight hidden sm:block ${
                      isCurrent ? 'text-cyan-300' : isPast ? 'text-slate-400' : 'text-slate-600'
                    }`}>
                      {stepCfg.short}
                    </span>
                  </div>
                  {idx < TIMELINE_STEPS.length - 1 && (
                    <div className={`flex-1 h-0.5 rounded-full transition-all ${
                      idx < currentStepIdx ? 'bg-cyan-500/50' : 'bg-slate-800'
                    }`} />
                  )}
                </React.Fragment>
              );
      ...
      {items.length > 0 ? (
        <div className="p-4 sm:p-5 space-y-2.5">
          {visibleItems.map((item, idx) => (
            <div key={item.id || idx} className="flex items-center gap-3 p-2.5 rounded-xl bg-slate-900/60 hover:bg-slate-900 transition-colors">
              <Link
                to={`/products/${item.product_id}`}
                className="flex-shrink-0 w-16 h-16 rounded-lg bg-slate-800 border border-slate-700 overflow-hidden"
              >
                <img
              ...
              <div className="flex-1 min-w-0">
                <Link
                  to={`/products/${item.product_id}`}
                  className="text-sm font-semibold text-white hover:text-cyan-300 line-clamp-2 transition-colors"
                  title={item.product_name}
                >
                  {item.product_name || `Sản phẩm #${item.product_id}`}
                </Link>
                <div className="flex items-center gap-3 mt-1 text-xs text-slate-400">
                  <span>Số lượng: <span className="text-cyan-300 font-bold">{item.quantity}</span></span>
              ...
              <div className="text-right flex-shrink-0">
                ...
                <p className="text-sm font-bold text-cyan-400">{formatVND(item.price * item.quantity)}</p>
              </div>
            </div>
          ))}
          {moreCount > 0 && (
            <button
              ...
              className="w-full py-2 text-xs text-cyan-300 hover:text-cyan-200 font-semibold bg-cyan-500/5 hover:bg-cyan-500/10 rounded-xl transition-colors flex items-center justify-center gap-1.5"
            >
              {expanded ? (
      ...
      <div className="px-4 sm:px-5 py-3 bg-slate-950/50 border-t border-slate-800 flex items-center justify-between gap-3">
        <div className="flex items-center gap-4 text-xs text-slate-400 flex-wrap">
          ...
          {order.shipping_fee > 0 && (
            <span className="flex items-center gap-1.5">
              <TruckIcon className="w-3.5 h-3.5 text-purple-400" />
...
function CancelOrderButton({ orderId, onCancelled }) {
  ...
  return (
    <>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm" onClick={() => !loading && setShowModal(false)}>
          <div className="glass-card rounded-2xl max-w-md w-full p-5 space-y-3 border border-slate-800" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-bold text-white">Hủy đơn hàng #{orderId}</h3>
              <button onClick={() => setShowModal(false)} disabled={loading} className="text-slate-400 hover:text-white disabled:opacity-50">
                <X className="w-5 h-5" />
              </button>
            </div>
            <p className="text-sm text-slate-400">Vui lòng chọn lý do hủy đơn hàng:</p>
            <div className="space-y-1">
              {presets.map((p) => (
                <label key={p} className="flex items-center gap-2 p-2 rounded-lg hover:bg-slate-800/50 cursor-pointer">
                  <input
                    type="radio"
                    name={`reason-${orderId}`}
                    value={p}
            ...
            {preset === 'Lý do khác' && (
              <textarea
                ...
                rows={3}
                className="w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded-xl text-sm text-slate-100 focus:outline-none focus:border-rose-500"
              />
            )}
            <div className="flex gap-2 pt-1">
              <button
                ...
                disabled={loading}
                className="flex-1 px-4 py-2.5 bg-slate-800 text-slate-200 rounded-xl hover:bg-slate-700 text-sm font-semibold disabled:opacity-50"
              >
                Đóng
              </button>
              <button
                ...
                className="flex-1 px-4 py-2.5 bg-rose-500 text-white rounded-xl hover:bg-rose-400 text-sm font-semibold disabled:opacity-50 flex items-center justify-center gap-2"
              >
...
function ResendBillForm({ orderId, onSuccess }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-2">
      <p className="text-[10px] text-slate-400 font-semibold">Gửi lại bill thanh toán</p>
      {preview ? (
        <div className="relative inline-block">
          <img src={preview} alt="Preview" className="h-24 rounded-lg border border-cyan-500/30" />
        ...
        <label className="flex items-center gap-2 px-3 py-2 border border-dashed border-slate-700 rounded-lg cursor-pointer hover:border-cyan-500/60 transition-colors">
          <Upload className="w-3.5 h-3.5 text-slate-500" />
          <span className="text-[10px] text-slate-400">Chọn ảnh bill</span>
          <input type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={handleFileChange} />
        </label>
      )}
      <button type="submit" disabled={!file || uploading} className="w-full py-1.5 bg-amber-500 text-slate-950 text-xs font-bold rounded-lg hover:bg-amber-400 disabled:opacity-40 flex items-center justify-center gap-1.5 transition-colors">
...
function OrderDetailModal({ order, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div className="glass-card rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto space-y-5" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-bold text-white">Chi Tiết Đơn Hàng</h2>
            <p className="text-xs text-slate-400">#{order.id} • {new Date(order.created_at).toLocaleDateString('vi-VN')}</p>
          </div>
                ...
                <React.Fragment key={step}>
                  <div className="flex flex-col items-center gap-1">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${
                      done ? 'bg-cyan-500/20 border-cyan-400 text-cyan-300' : 'bg-slate-900 border-slate-700 text-slate-500'
                    }`}>
                      <StepIcon className="w-4 h-4" />
                    </div>
                    <span className={`text-[9px] font-semibold text-center leading-tight ${done ? 'text-cyan-300' : 'text-slate-600'}`}>
                      {cfg.label.split(' ')[0]}
                    </span>
                  </div>
                  {idx < TIMELINE_STEPS.length - 1 && (
                    <div className={`flex-1 h-0.5 mb-5 mx-1 rounded ${idx < currentIdx ? 'bg-cyan-500/60' : 'bg-slate-800'}`} />
                  )}
                </React.Fragment>
              );
            })}
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="p-3 rounded-xl bg-slate-900/60 border border-slate-800">
            ...
            <div className="flex items-center justify-between mt-1">
              <p className="text-sm font-semibold text-cyan-300">{PAYMENT_LABEL[order.payment_method] || order.payment_method}</p>
              {isBillApproved && (
              ...
              {!billStatus && recordStatus === 'cancelled' && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-700/40 text-slate-300 border border-slate-700">
                  Đã hủy
                </span>
              )}
            </div>
          ...
          {showBillSection && (
            <div className="col-span-2 p-3 rounded-xl bg-slate-900/60 border border-slate-800">
              ...
              {order.bill_image_url ? (
                <div className="mt-2 space-y-2">
                  <div className="relative inline-block">
                    <img
                      ...
                      alt="Bill thanh toán"
                      className="max-w-full h-auto max-h-48 rounded-xl border border-slate-700 cursor-pointer hover:border-cyan-500 transition-colors"
                    ...
                    />
                    <div data-bill-fallback className="hidden flex-col items-center justify-center p-4 bg-amber-500/10 border border-amber-500/30 rounded-xl text-center min-h-[8rem]">
                      ...
                      <p className="text-[10px] text-slate-400 mt-1">File có thể đã bị lỗi. Vui lòng upload lại bill.</p>
                      <a href={getBackendUrl(order.bill_image_url)} target="_blank" rel="noreferrer" className="text-[10px] text-cyan-400 hover:underline mt-1">Mở trong tab mới</a>
                    </div>
                  </div>
                  {!isBillApproved && (
                ...
                <div className="mt-2 space-y-2">
                  <p className="text-xs text-slate-400">Chưa có ảnh bill. Vui lòng tải lên để xác nhận thanh toán.</p>
                  <ResendBillForm orderId={order.id} onSuccess={() => onClose && onClose()} />
                </div>
              ) : (
        ...
        <div>
          ...
          <div className="mt-2 divide-y divide-slate-800/60 rounded-xl border border-slate-800 overflow-hidden">
            {items.map((item, idx) => (
              <div key={idx} className="flex items-center gap-3 p-3 bg-slate-900/40">
                <img
                ...
                <div className="text-right">
                  <p className="text-sm font-bold text-cyan-400">{formatPrice(item.price * item.quantity)}</p>
        ...
        <div className="space-y-1.5 p-3 rounded-xl bg-slate-900/60 border border-slate-800">
          ...
          <div className="flex justify-between text-base font-bold text-white pt-2 border-t border-slate-800">
            ...
            <span className="text-cyan-400">{formatPrice(order.final_amount)}</span>
          </div>
        </div>
  ...
  const field = (name, label, type = 'text', placeholder = '') => (
    <div>
      ...
      <input
        ...
        className={`w-full px-3 py-2.5 bg-slate-950 border rounded-xl text-xs text-slate-100 focus:outline-none focus:border-cyan-500 ${errors[name] ? 'border-rose-500' : 'border-slate-800'}`}
      />
    ...
    <div>
      ...
      <div className="relative">
        <select
          ...
          className={`w-full px-3 py-2.5 bg-slate-950 border rounded-xl text-xs text-slate-100 focus:outline-none focus:border-cyan-500 appearance-none pr-8 ${errors[errKey || name] ? 'border-rose-500' : 'border-slate-800'} ${name === 'ward_id' && !form.district_id ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
      ...
      <div className="glass-card rounded-2xl p-6 w-full max-w-md space-y-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" checked={form.is_default} onChange={e => setForm({ ...form, is_default: e.target.checked })} className="accent-cyan-500 w-4 h-4" />
            <span className="text-xs text-slate-300 font-semibold">Đặt làm địa chỉ mặc định</span>
          </label>
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 py-2.5 bg-slate-800 text-slate-300 rounded-xl text-xs font-semibold hover:bg-slate-700">Hủy</button>
            <button type="submit" disabled={loading} className="flex-1 py-2.5 bg-cyan-500 text-slate-950 font-bold rounded-xl text-xs hover:bg-cyan-400 disabled:opacity-50 flex items-center justify-center gap-2">
    ...
    <div>
      ...
      <div className="relative">
        <input
          ...
          value={value}
          onChange={onChange}
          className="w-full px-4 py-2.5 bg-slate-900 border border-slate-800 rounded-xl text-sm text-slate-100 focus:outline-none focus:border-cyan-500 pr-10"
        />
        <button type="button" onClick={() => setShowPass({ ...showPass, [name]: !showPass[name] })} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-cyan-400">
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-3">
          <div className="glass-card rounded-2xl p-3 space-y-1 sticky top-4">
            {[
              ...
              >
                ...
                {badge !== null && badge > 0 && (
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${activeTab === key ? 'bg-slate-950/30 text-slate-950' : 'bg-slate-800 text-slate-400'}`}>
                    {badge}
                  </span>
                )}
              </button>
        ...
        <div className="lg:col-span-9 space-y-6">
          ...
          {activeTab === 'coupons' && (
            <div className="glass-card rounded-3xl p-8 space-y-5">
              <div className="border-b border-slate-800 pb-3">
                ...
                <p className="text-xs text-slate-400 mt-1">Sao chép mã và dùng khi đặt hàng</p>
              </div>
              ...
              {couponsLoading ? (
                ...
                <div className="p-12 text-center space-y-3 bg-slate-900/50 rounded-2xl border border-slate-800">
                  ...
                  <p className="text-slate-400 font-semibold">Hiện không có mã giảm giá nào khả dụng.</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {coupons.map((coupon) => {
                    ...
                    return (
                      <div
                        key={coupon.id}
                        className={`relative p-5 rounded-2xl bg-gradient-to-br from-amber-500/10 to-cyan-500/5 border border-amber-500/20 space-y-3 overflow-hidden ${!isValid ? 'opacity-50' : ''}`}
                      >
                        <div className="absolute top-0 right-0 w-20 h-20 bg-amber-500/10 rounded-full -translate-y-1/2 translate-x-1/2" />

                        <div className="flex items-center gap-3">
                          <div className="flex items-center justify-center w-14 h-14 rounded-2xl bg-amber-500/20 border border-amber-500/30">
                            <span className="text-xl font-black text-amber-400">-{coupon.discount_percent}%</span>
                          </div>
                          <div>
                            <div className="flex items-center gap-2">
                              <p className="text-xs text-slate-400 font-semibold">MÃ GIẢM GIÁ</p>
                              {!isValid && (
                            ...
                            {coupon.description && (
                              <p className="text-xs text-slate-400 line-clamp-1">{coupon.description}</p>
                            )}
                          </div>
                        </div>
                          ...
                          {coupon.usage_per_user != null && coupon.usage_per_user > 0 && (
                            <div className="flex items-center gap-2">
                              <span className={`w-1.5 h-1.5 rounded-full ${(() => {
                                ...
                                return remaining > 0 ? 'bg-purple-400' : 'bg-rose-400';
                              })()} `} />
                              {(() => {
                                const limit = coupon.usage_per_user;
                                const used = coupon.user_used_count || 0;
                          ...
                          return remaining > 0 ? (
                            <div className="flex gap-2">
                              <button
                                ...
                                className="flex-1 py-2.5 bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/40 text-amber-300 font-bold rounded-xl text-xs flex items-center justify-center gap-2 transition-colors"
                              >
                                <Copy className="w-3.5 h-3.5" /> Sao Chép
                              </button>
                              <Link
                                to="/checkout"
                                className="flex-1 py-2.5 bg-cyan-500/20 hover:bg-cyan-500/30 border border-cyan-500/40 text-cyan-300 font-bold rounded-xl text-xs flex items-center justify-center gap-2 transition-colors"
                              >
                                Dùng Ngay
                              </Link>
                            </div>
                          ) : (
                            <div className="py-2 text-center text-xs text-rose-400 bg-rose-500/10 border border-rose-500/20 rounded-xl">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-500">
                        Tổng <strong className="text-slate-300">{orderPagination.totalItems || orderList.length}</strong> đơn
                        {orderPagination.totalPages > 1 && (
                          <> · Trang <strong className="text-slate-300">{orderPage}</strong>/{orderPagination.totalPages}</>
                        )}
                      </p>
                    </div>
                  </div>
                ...
                <div className="flex flex-col sm:flex-row gap-2.5">
                    <div className="flex-1 relative">
                      <input
                        ...
                        className="w-full pl-9 pr-4 py-2 bg-slate-900 border border-slate-800 rounded-xl text-sm text-slate-100 focus:outline-none focus:border-cyan-500"
                      />
                    ...
                    <div className="relative min-w-[160px]">
                      <select
                        ...
                        className="w-full pl-3 pr-9 py-2 bg-slate-900 border border-slate-800 rounded-xl text-sm text-slate-100 focus:outline-none focus:border-cyan-500 appearance-none cursor-pointer"
                      >
              ...
              {ordersLoading ? (
                ...
                orderList.length === 0 ? (
                  <div className="glass-card rounded-3xl p-12 text-center space-y-3">
                    <div className="w-20 h-20 mx-auto rounded-full bg-slate-800/60 flex items-center justify-center">
                      <ShoppingBag className="w-10 h-10 text-slate-600" />
                    </div>
                    <p className="text-slate-300 font-bold text-lg">Bạn chưa có đơn hàng nào.</p>
                    <p className="text-xs text-slate-500">Hãy khám phá các sản phẩm hấp dẫn của chúng tôi!</p>
                    <a href="/products" className="inline-block px-6 py-2.5 bg-gradient-to-r from-cyan-400 to-sky-400 text-slate-950 font-bold rounded-xl text-xs hover:shadow-lg hover:shadow-cyan-500/25 transition-all">
                      Khám Phá Sản Phẩm
                    </a>
                  </div>
                ) : (
                  <div className="glass-card rounded-3xl p-12 text-center space-y-3">
                    ...
                    <p className="text-slate-300 font-bold text-lg">Không tìm thấy đơn hàng nào.</p>
                    <button
                      onClick={() => { setOrderSearch(''); setOrderFilter('all'); }}
                      className="inline-block px-5 py-2.5 bg-slate-800 text-slate-300 font-semibold rounded-xl text-xs hover:bg-slate-700"
                    >
                      Xóa bộ lọc
                    </button>
                  </div>
          ...
          {activeTab === 'addresses' && (
            <div className="glass-card rounded-3xl p-8 space-y-5">
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <h2 className="text-xl font-bold text-white flex items-center gap-2">
                  <MapPin className="w-5 h-5 text-cyan-400" /> Sổ Địa Chỉ Giao Hàng
                </h2>
                <button
                  onClick={() => setAddressModal({})}
                  className="px-4 py-2 bg-cyan-500 text-slate-950 font-bold rounded-xl text-xs flex items-center gap-1.5 hover:bg-cyan-400"
                >
              ...
              {addressesLoading ? (
                ...
                <div className="p-12 text-center space-y-3 bg-slate-900/50 rounded-2xl border border-slate-800">
                  ...
                  <p className="text-slate-400 font-semibold">Chưa có địa chỉ giao hàng nào.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {addresses.map((addr) => (
                    <div key={addr.id} className={`p-5 rounded-2xl bg-slate-900/70 border space-y-2 relative ${
                      addr.is_default ? 'border-cyan-500/40' : 'border-slate-800'
                    }`}>
                      {addr.is_default && (
                        <span className="absolute top-3 right-3 px-2.5 py-0.5 bg-cyan-500/20 text-cyan-300 text-[10px] font-bold rounded-full border border-cyan-500/30">
                          Mặc Định
                        </span>
                      )}
                      <div className="flex items-center justify-between">
                      ...
                      <p className="text-xs text-slate-300 font-semibold">{addr.phone}</p>
              ...
              <form onSubmit={handleSaveProfile} className="space-y-4 max-w-xl">
                ...
                <div>
                  ...
                  <input
                    type="email" disabled
                    value={user?.email || ''}
                    className="w-full px-4 py-2.5 bg-slate-950 border border-slate-800/60 rounded-xl text-sm text-slate-500 cursor-not-allowed"
                  />
                </div>
                <div>
                ...
                <button
                  ...
                  className="px-6 py-3 bg-cyan-500 text-slate-950 font-bold rounded-xl hover:bg-cyan-400 transition-colors flex items-center gap-2 disabled:opacity-50"
                >
          ...
          {activeTab === 'security' && (
            <div className="glass-card rounded-3xl p-8 space-y-6">
              <h2 className="text-xl font-bold text-white border-b border-slate-800 pb-3 flex items-center gap-2">
                <Key className="w-5 h-5 text-cyan-400" /> Đổi Mật Khẩu
              </h2>
              ...
              <form onSubmit={handlePasswordSubmit} className="space-y-4 max-w-md">
                ...
                <button
                  ...
                  className="px-6 py-3 bg-gradient-to-r from-cyan-400 to-sky-400 text-slate-950 font-bold rounded-xl hover:opacity-90 flex items-center gap-2 disabled:opacity-50"
                >
